...
import com.lgh.uvccamera.GPYUsbDevice;
...
public class UsbMonitor implements IMonitor {
    ...
    public void checkDevice() {
        ...
        ArrayList<UsbDevice> usbDevices = getUsbCameraDevice();
        ArrayList<GPYUsbDevice> gpyUsbDevices = new ArrayList<GPYUsbDevice>();
        if (usbDevices == null) {
            LogUtil.e("usbDevices is null");
            return;
        }
        LogUtil.d("usbDevices size: " + usbDevices.size());
        gpyUsbDevices.add(new GPYUsbDevice(0x050a,0x1e4e));
        gpyUsbDevices.add(new GPYUsbDevice(0x050a,0x1b17));
        gpyUsbDevices.add(new GPYUsbDevice(0x0591,0x1b17));
        gpyUsbDevices.add(new GPYUsbDevice(0x10a0,0x1b17));
        for (UsbDevice usbDevice: usbDevices) {
            Iterator<GPYUsbDevice> iter = gpyUsbDevices.iterator();
            while (iter.hasNext()){
                GPYUsbDevice devices = (GPYUsbDevice) iter.next();
                if((usbDevice.getProductId() == devices.getPid()) && (usbDevice.getVendorId() == devices.getVid())){
                    mConnectCallback.onAttached(usbDevice);
                    mConfig.setProductId(devices.getPid());
                    mConfig.setVendorId(devices.getVid());
                    break;
                }
            }
        }
     ...
     * @return
     */
    public ArrayList<UsbDevice> getUsbCameraDevice() {
        ...
        return usbDevices.isEmpty() ? null : usbDevices;
    }
